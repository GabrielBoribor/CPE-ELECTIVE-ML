...
class InventoryGUI:
    ...
    def create_widgets(self):
        ...
        quit_button = ttk.Button(main_frame, text="Quit", command=self.root.quit, style='Custom.TButton')
        quit_button.grid(row=2, column=2, padx=20, pady=10)
    ...
    def add_items(self):
        ...
        add_window.transient(self.root)  # Set the "Add Items" window to be transient with respect to the root window
        ...
        item_entry.focus_set()  # Set focus to the entry widget

        # Function to deselect text when entry widget receives focus
    ...
    def remove_items(self):
        item_index = simpledialog.askinteger("Remove Items", "Enter the number of the item to remove:")
        if item_index:
            if 0 < item_index <= len(self.item_names):
                item = self.item_names[item_index - 1]
                quantity = simpledialog.askinteger("Remove Items", f"Enter quantity to remove for {item}:")
                if quantity:
                    inventory_key = f"{item_index}. {item}"
